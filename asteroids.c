...
#define HIT_SCORE_REDUCTION 500
...
static time_t timeHit, startTime; 
static int score, hitDeduction = 0;
static char elapsedTime[9];
...
/*	ftglRenderFont(font, "Hello World!", FTGL_RENDER_ALL);*/
	...
	//sprintf(scoreString, "Score : %d", score.score);
    //strcat(score.Char, scoreString);
    sprintf(scoreString,"Score : %6d",score);
    //strcat(scoreString,score.scoreChar);	
	
	drawText(0, yMax-5, scoreString);
	
	//drawtext(xMax - 10, yMax-5, elapsedTime);
	
	if(invulnerable){
		drawText(50,50, "YOU GOT HIT");
		if(score > 100 && !hitDeduction){
			score -= HIT_SCORE_REDUCTION;
			hitDeduction = 1;
		}
		else if(!hitDeduction){
			score = 0;
			hitDeduction = 1;
		}
	}
...
/*	strftime(elapsedTime, sizeof(elapsedTime), "%H:%M:%S", time_info);*/
/*	*/
/*}*/
